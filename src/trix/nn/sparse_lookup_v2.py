...
class SparseLookupFFNv2(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer(
            "cluster_assignments", torch.arange(num_tiles) // tiles_per_cluster
        )
    ...
    def route(
        ...
        all_scores = (
            torch.zeros(batch_size, self.num_tiles, device=device)
            if return_scores
            else None
        )
        ...
        for c in range(self.num_clusters):
            ...
            if return_scores:
                # NOTE: `all_scores[mask]` is not a view; assign via explicit row indices.
    ...
    def _compute_aux_losses(self, tile_idx: torch.Tensor, total: int) -> Dict:
        ...
        # Island regularizers
        ternary_loss = (
            self.compute_ternary_loss()
            if self.ternary_weight > 0
            else torch.tensor(0.0, device=device)
        )
        sparsity_loss = (
            self.compute_sparsity_loss()
            if self.sparsity_weight > 0
            else torch.tensor(0.0, device=device)
        )
        diversity_loss = (
            self.compute_diversity_loss()
