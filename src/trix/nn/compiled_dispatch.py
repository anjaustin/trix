...
class CompiledDispatch(nn.Module):
    ...
    def profile(self, class_id: int) -> ProfileStats:
        ...
        if not hasattr(self.ffn, "claim_matrix"):
            raise RuntimeError(
                "FFN must have claim tracking enabled. "
        ...
        tile_distribution = {
            i: int(class_counts[i]) for i in range(num_tiles) if class_counts[i] > 0
        }
    ...
    def compile_from_profile(
        self, stats: ProfileStats, min_confidence: float = 0.5
    ) -> Optional[CompiledEntry]:
    ...
    def _execute_compiled(
        self, x: torch.Tensor, entry: CompiledEntry
    ) -> Tuple[torch.Tensor, Dict, Dict]:
        ...
        # Preferred: FFN provides an explicit forced-tile execution path.
        if hasattr(self.ffn, "forward_forced_tile"):
            output, routing_info, aux_losses = self.ffn.forward_forced_tile(
                x, tile_idx=tile_idx
            )
            return output, routing_info, aux_losses

        # Legacy: direct tile access when FFN exposes a tiles bank.
        if hasattr(self.ffn, "tiles"):
            tile = self.ffn.tiles[tile_idx]
            output = tile(x)
        else:
            # Last resort: fall back to dynamic routing (not truly compiled).
            output, routing_info, aux_losses = self.ffn(x)
            return output, {"tile_idx": torch.tensor([[tile_idx]])}, aux_losses

        routing_info = {
            "tile_idx": torch.full(
                (x.shape[0], x.shape[1] if x.dim() > 2 else 1),
                tile_idx,
                dtype=torch.long,
                device=x.device,
            ),
    ...
    def __repr__(self):
        ...
        return (
            f"CompiledDispatch(compiled_classes={stats['num_compiled_classes']}, "
