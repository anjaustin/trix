...
def _find_library() -> Optional[Path]:
    """Search for the TriX shared library in common locations."""

    if sys.platform == "darwin":
        lib_names = ["libtrix.dylib", "libtrix.so"]
    elif os.name == "nt":
        lib_names = ["trix.dll", "libtrix.dll"]
    else:
        lib_names = ["libtrix.so"]

    search_dirs = [
        Path(__file__).parent / "build",
        Path(__file__).parent,
        # repo_root/build (sometimes used by docs)
        Path(__file__).parent.parent.parent.parent.parent / "build",
        # cmake out-of-tree build
        Path(__file__).parent / "build" / "Release",
        Path(__file__).parent / "build" / "Debug",
    ]

    search_paths = [d / name for d in search_dirs for name in lib_names]
...
def _load_library(*, raise_on_missing: bool = True):
    """Load the TriX C library (ctypes)."""
    ...
    if _LIB_PATH is None:
        if raise_on_missing:
            raise RuntimeError(
                "TriX native library not found. Build with:\n"
                "  cmake -S src/trix/kernel -B src/trix/kernel/build\n"
                "  cmake --build src/trix/kernel/build\n\n"
                "Expected one of: libtrix.so (linux), libtrix.dylib (macOS), trix.dll (windows)"
            )
        return None
...
def is_neon_available() -> bool:
    ...
    try:
        _load_library(raise_on_missing=True)
        return True
    except RuntimeError:
        return False


def _pack_weights_python(weights: torch.Tensor) -> torch.Tensor:
    """Portable pack_weights implementation (no native library required)."""
    rows, cols = weights.shape
    weights_f32 = weights.cpu().to(torch.float32).contiguous()

    # Match kernel thresholds: w > 0.5 => +1 (01), w < -0.5 => -1 (10), else 0 (00)
    codes = torch.zeros((rows, cols), dtype=torch.uint8)
    codes[weights_f32 > 0.5] = 0x01
    codes[weights_f32 < -0.5] = 0x02

    packed_cols = (cols + 3) // 4
    pad = packed_cols * 4 - cols
    if pad:
        codes = torch.nn.functional.pad(codes, (0, pad), mode="constant", value=0)

    codes4 = codes.view(rows, packed_cols, 4)
    packed = (
        (codes4[:, :, 0])
        | (codes4[:, :, 1] << 2)
        | (codes4[:, :, 2] << 4)
...
def _unpack_weights_python(packed: torch.Tensor, rows: int, cols: int) -> torch.Tensor:
    """Portable unpack_weights implementation (no native library required)."""
    packed_u8 = packed.cpu().to(torch.uint8).contiguous()
    packed_cols = (cols + 3) // 4
    if packed_u8.shape != (rows, packed_cols):
        packed_u8 = packed_u8.view(rows, packed_cols)
    ...
    b2 = (packed_u8 >> 4) & 0x03
    b3 = (packed_u8 >> 6) & 0x03
    codes = torch.stack([b0, b1, b2, b3], dim=-1).view(rows, packed_cols * 4)

    out = torch.zeros((rows, packed_cols * 4), dtype=torch.float32)
    out[codes == 0x01] = 1.0
    out[codes == 0x02] = -1.0
...
def pack_weights(weights: torch.Tensor) -> torch.Tensor:
    ...
    """
    lib = _load_library(raise_on_missing=False)
    if lib is None:
        return _pack_weights_python(weights)
    ...
    lib.pack_weights(weights_f32.data_ptr(), packed.data_ptr(), rows, cols)
...
def unpack_weights(packed: torch.Tensor, rows: int, cols: int) -> torch.Tensor:
    ...
    lib = _load_library(raise_on_missing=False)
    if lib is None:
        return _unpack_weights_python(packed, rows, cols)

    output = torch.zeros((rows, cols), dtype=torch.float32)

    lib.unpack_weights(packed.data_ptr(), output.data_ptr(), rows, cols)
...
def trix_forward(
    ...
    lib = _load_library(raise_on_missing=False)
    if lib is None:
        # Portable reference implementation (uses unpack + matmul).
        batch, in_features = x.shape
        w = unpack_weights(packed_weights, out_features, in_features)
        out = (x.cpu().to(torch.float32) @ w.t()) * scales.cpu().to(torch.float32)

        out_per_tile = out_features // num_tiles
        gate_f32 = gate.cpu().to(torch.float32)
        for t in range(num_tiles):
            start = t * out_per_tile
            end = start + out_per_tile
            out[:, start:end] = out[:, start:end] * gate_f32[:, t : t + 1]
...
class TriXLinear(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, gate: torch.Tensor, use_gate: bool = False
    ) -> torch.Tensor:
        """
        Forward pass.
