...
class TestFP4Atoms:
    ...
    def test_sum_atom_exhaustive(self):
        ...
        for a, b, c in product([0, 1], repeat=3):
            ...
            assert actual == expected, (
                f"SUM({a},{b},{c}) = {actual}, expected {expected}"
            )
    ...
    def test_carry_atom_exhaustive(self):
        ...
        for a, b, c in product([0, 1], repeat=3):
            ...
            actual = int(y[0, 0].item() > 0.5)
            assert actual == expected, (
                f"CARRY({a},{b},{c}) = {actual}, expected {expected}"
            )
...
class TestAdderExhaustive:
    ...
    def test_full_adder_exhaustive(self):
        ...
        for a, b, cin in product([0, 1], repeat=3):
            ...
            assert actual_sum == expected_sum, (
                f"SUM({a},{b},{cin}) = {actual_sum}, expected {expected_sum}"
            )
            assert actual_cout == expected_cout, (
                f"CARRY({a},{b},{cin}) = {actual_cout}, expected {expected_cout}"
            )
    ...
    def test_8bit_adder_exhaustive(self):
        ...
        def add_8bit(a: int, b: int) -> int:
            ...
            for i in range(8):
                x = torch.tensor(
                    [[float(a_bits[i]), float(b_bits[i]), float(carry)]],
                    dtype=torch.float32,
                )
        ...
        assert len(errors) == 0, (
            f"8-bit adder failed on {len(errors)} cases. First 5: {errors[:5]}"
        )
...
class TestAtomComposition:
    ...
    def test_xor_chain(self):
        ...
        for a, b, c in product([0, 1], repeat=3):
            ...
            expected = a ^ b ^ c
            assert actual == expected, (
                f"XOR(XOR({a},{b}),{c}) = {actual}, expected {expected}"
            )
...
class TestNumericalStability:
    ...
    def test_dft_large_values(self):
        ...
        np.testing.assert_allclose(
            np.array(out_re) + 1j * np.array(out_im), expected, rtol=1e-9
        )
...
class TestAtomComposition:
    ...
    def test_and_or_chain(self):
        ...
        for a, b, c in product([0, 1], repeat=3):
            ...
            assert result == expected, (
                f"AND_OR({a},{b},{c}) = {result}, expected {expected}"
            )
    ...
    def test_xor_xor_chain(self):
        ...
        for a, b, c in product([0, 1], repeat=3):
            ...
            expected = (a ^ b) ^ c
            assert result == expected, (
                f"XOR_XOR({a},{b},{c}) = {result}, expected {expected}"
            )
    ...
    def test_full_adder_from_atoms(self):
        ...
        for a, b, cin in product([0, 1], repeat=3):
            ...
            assert sum_result == expected_sum, (
                f"SUM({a},{b},{cin}) = {sum_result}, expected {expected_sum}"
            )
            assert carry_result == expected_carry, (
                f"CARRY({a},{b},{cin}) = {carry_result}, expected {expected_carry}"
            )
